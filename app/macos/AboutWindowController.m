...
- (instancetype)init {
    NSWindow *win = [[NSWindow alloc] initWithContentRect:NSMakeRect(0, 0, 420, 360)
...
- (void)build {
    ...
    NSTextField *credits = [NSTextField wrappingLabelWithString:
        @"Editor/Assembler and Simulator for the Motorola 68000.\n\n"
         ...
         "Eric Nelson.\n\n"
         "Ported to a native macOS application — C99 core (assembler + "
         "simulator) with a Cocoa/AppKit interface."];
    ...
    for (NSView *v in @[logo, title, ver, credits]) [c addSubview:v];
