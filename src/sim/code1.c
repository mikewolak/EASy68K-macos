...
int	MOVEP()
{
  ...
  mem_request (&PC, (int32_t) WORD_MASK, &temp);
  from_2s_comp (temp, (int32_t) WORD_MASK, &temp);
  ...
  for (;count > 0; count--)
  {
    disp = 8 * (count - 1);
    if (direction)
      mem_put ( (int32_t)((Dx >> disp) & BYTE_MASK) , address, (int32_t) BYTE_MASK);
    else {
      mem_req (address, (int32_t) BYTE_MASK, &temp);
      switch  (count) {
...
if (size == WORD_MASK)
	sign_extend ((int)EV1, (int32_t) WORD_MASK, &EV1);
...
int	MOVE_FR_SR()
{
  ...
  int error = eff_addr((int32_t)WORD_MASK, DATA_ALT_ADDR, true);
  if (error)              // if address error
    return error;         // return error code

  put (EA1, (int32_t) SR, (int32_t) WORD_MASK);
...
int	MOVE_TO_CCR()
{
  int error = eff_addr((int32_t)WORD_MASK, DATA_ADDR, true);
  if (error)              // if address error
    return error;         // return error code

  put ((int32_t *)&SR, EV1, (int32_t) BYTE_MASK);
  inc_cyc (12);
...
int	MOVE_TO_SR()
{
  ...
  put ((int32_t *)&SR, EV1, (int32_t) WORD_MASK);
  inc_cyc (12);
  return SUCCESS;
}
...
int	MOVEM()
{
  ...
  int32_t	size, mask_list, temp;
  uint  saveAn = A[a_reg(inst & 0x7)];  // save possible destination (CK 5-1-2007)

  mem_request (&PC, (int32_t) WORD_MASK, &mask_list);
  ...
  address = (int32_t) ( (int32_t)EA1 - (int32_t)&memory[0]);
  ...
  for (counter = 0; counter < 16; counter++) {
	...
	if (size == WORD_MASK) {        // if size is WORD_MASK then sign-extend
	  mem_req (address, (int32_t) WORD_MASK, &temp);
	  sign_extend ((int) temp, WORD_MASK, &temp);   // ck 8-10-2005
	} else                          // ck added this mem_req
	  mem_req (address, (int32_t) LONG_MASK, &temp);
	if (counter < 8)
	  D[counter] = temp;
	else
...
sign_extend ((int)source, (int32_t) BYTE_MASK, &D[reg]);
sign_extend ((int)D[reg], (int32_t) WORD_MASK, &D[reg]);
