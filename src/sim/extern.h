...
extern int32_t     D[D_REGS], OLD_D[D_REGS], A[A_REGS], OLD_A[A_REGS];
...
extern int	inst;
extern int32_t	*EA1, *EA2;
extern int32_t	EV1, EV2;

extern int32_t	source, dest, result;

extern int32_t    	global_temp;		/* to hold an immediate data operand */
...
extern bool bpRead;
extern bool bpWrite;
extern int32_t * readEA;
extern int32_t * writeEA;
...
// Note: the original Windows build kept Win32 multi-monitor / DirectSound API
// function pointers here (ChangeDisplaySettingsEx etc.). Full-screen output and
// sound are reimplemented natively in the macOS GUI layer, so those Win32-only
// declarations are intentionally omitted from the portable core.
