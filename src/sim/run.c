...
int eff_addr (int32_t size, int mask, int add_times)
{
  ...
  int	bwinc, linc;
  int32_t	ext, temp_ext, inc_size, ind_reg, *value, disp;
  ...
  switch (mode) {               // switch on effective address mode
    ...
    case 2 :                                    // [An]
      if (mask & bit_3) {
	reg = a_reg(reg);
	//value = (int32_t *) &memory[A[reg]];
        if (move_operation) { // choose EA2 in case of MOVE dest effective address
          EA2 = (int32_t *) &memory[A[reg] & ADDRMASK];
          error = mem_req(A[reg], size, &EV2);
        } else {
          EA1 = (int32_t *) &memory[A[reg] & ADDRMASK];
    ...
    case 5 :                                    // d[An]
      if (mask & bit_6)	{
	reg = a_reg(reg);
	mem_request (&PC, (int32_t) WORD_MASK, &ext);
	from_2s_comp (ext, (int32_t) WORD_MASK, &ext);
	//value = (int32_t *) &memory[A[reg] + ext];
        if (move_operation) { // choose EA2 in case of MOVE dest effective address
          EA2 = (int32_t *) &memory[(A[reg] + ext) & ADDRMASK];
          error = mem_req(A[reg] + ext, size, &EV2);
        } else {
          EA1 = (int32_t *) &memory[(A[reg] + ext) & ADDRMASK];
	...
	from_2s_comp (disp, (int32_t) WORD_MASK, &disp);
	...
	if (!(ext & 0x0800)) {
	  ...
	  from_2s_comp (ind_reg, (int32_t) LONG_MASK, &ind_reg);
	}
	//value = (int32_t *) (&memory[A[reg] + disp + ind_reg]);
        if (move_operation) { // choose EA2 in case of MOVE dest effective address
          EA2 = (int32_t *) &memory[(A[reg] + disp + ind_reg) & ADDRMASK];
          error = mem_req(A[reg] + disp + ind_reg, size, &EV2);
        } else {
          EA1 = (int32_t *) &memory[(A[reg] + disp + ind_reg) & ADDRMASK];
	...
	case 0 :                                // Abs.W
          if (mask & bit_8) {
	    mem_request (&PC, (int32_t) WORD_MASK, &ext);  // get address from instruction
            if (ext >= 0x8000)          // if address is negative word  ck 1-11-2008
              ext = 0xFFFF0000 | ext;   // sign extend (corrected ck 6-23-2009)
            if (move_operation) { // choose EA2 in case of MOVE dest effective address
              EA2 = (int32_t *) &memory[ext & ADDRMASK];              // get effective address
              error = mem_req(ext, size, &EV2);         // read data
            } else {
              EA1 = (int32_t *) &memory[ext & ADDRMASK];
	...
	case 1 :                                // Abs.L
	    ...
	    //value = (int32_t *) &memory[ext & ADDRMASK];
            if (move_operation) { // choose EA2 in case of MOVE dest effective address
              EA2 = (int32_t *) &memory[ext & ADDRMASK];
	...
	case 2 :                                // d[PC]
	    ...
	    mem_request (&PC, (int32_t) WORD_MASK, &ext);
	    from_2s_comp (ext, (int32_t) WORD_MASK, &ext);
	    //value = (int32_t *) &memory[PC + ext - 2];
            if (move_operation) { // choose EA2 in case of MOVE dest effective address
              EA2 = (int32_t *) &memory[(PC + ext - 2) & ADDRMASK];
              error = mem_req(PC + ext - 2, size, &EV2);
            } else {
              EA1 = (int32_t *) &memory[(PC + ext - 2) & ADDRMASK];
	...
	case 3 :                                // d[PC, Xi]
	    ...
	    ext = ext & 0x00ff;
	    //value = (int32_t *) (&memory[PC - 2 + disp + ind_reg]);
            if (move_operation) { // choose EA2 in case of MOVE dest effective address
              EA2 = (int32_t *) &memory[(PC - 2 + disp + ind_reg) & ADDRMASK];
              error = mem_req(PC - 2 + disp + ind_reg, size, &EV2);
            } else {
              EA1 = (int32_t *) &memory[(PC - 2 + disp + ind_reg) & ADDRMASK];
	...
	case 4 :                                // Imm
	    ...
	    //value = &global_temp;
            if (move_operation) { // choose EA2 in case of MOVE dest effective address
              EA2 = (int32_t *) &global_temp;
              value_of (EA2, &EV2, size);
            } else {
              EA1 = (int32_t *) &global_temp;
              value_of (EA1, &EV1, size);
            }
	    bwinc = 4;
	    linc = 8;
...
int eff_addr_noread (int32_t size, int mask, int add_times)
{
  ...
  switch (mode) {               // switch on effective address mode
    ...
    case 2 :                                    // [An]
      if (mask & bit_3) {
	reg = a_reg(reg);
	value = (int32_t *) &memory[A[reg] & ADDRMASK];
	bwinc = 4;
	linc = 8;
	legal = true;
      }
	...
	mem_request (&PC, (int32_t) WORD_MASK, &ext);
	from_2s_comp (ext, (int32_t) WORD_MASK, &ext);
	value = (int32_t *) &memory[(A[reg] + ext) & ADDRMASK];
	bwinc = 8;
	linc = 12;
	legal = true;
      }
	...
	value = (int32_t *) (&memory[(A[reg] + disp + ind_reg) & ADDRMASK]);
	bwinc = 10;
	linc = 14;
	legal = true;
      }
	...
	case 1 :                                // Abs.L
	    ...
	    //mem_request (&PC, LONG_MASK, &temp_ext);  //ck
	    //ext = ext * 0xffff + temp_ext;            //ck
	    //value = (int32_t *) &memory[ext & ADDRMASK]; //ck 2-21-03
	    value = (int32_t *) &memory[ext & ADDRMASK];
            bwinc = 12;
	    linc = 16;
	    legal = true;
	...
	case 2 :                                // d[PC]
	    ...
	    mem_request (&PC, (int32_t) WORD_MASK, &ext);
	    from_2s_comp (ext, (int32_t) WORD_MASK, &ext);
	    value = (int32_t *) &memory[(PC + ext - 2) & ADDRMASK];
	    bwinc = 8;
	    linc = 12;
	    legal = true;
	  }
	...
	case 3 :                                // d[PC, Xi]
	    ...
	    ext = ext & 0x00ff;
	    value = (int32_t *) (&memory[(PC - 2 + disp + ind_reg) & ADDRMASK]);
	    bwinc = 10;
	    linc = 14;
	    legal = true;
	  }
...
int exec_inst()
{
...

  {
    ...
    exec_result = mem_request (&PC, (int32_t) WORD_MASK, (int32_t *)&inst);
    if ( !(exec_result) )
    {
...
void exceptionHandler(int clas, int32_t loc, int r_w)
{
  ...
  if ( (clas == 1) || (clas == 2)) {
    ...
    put ((int32_t *)&memory[A[8] & ADDRMASK], OLD_PC, LONG_MASK);
    A[8] -= 2;
    put ((int32_t *)&memory[A[8] & ADDRMASK], (int32_t) SR, (int32_t) WORD_MASK);
    ...
    put ((int32_t *)&memory[A[8] & ADDRMASK], OLD_PC+2, LONG_MASK);   // OLD_PC+2 to match MECB
    A[8] -= 2;
    put ((int32_t *)&memory[A[8] & ADDRMASK], (int32_t) SR, (int32_t) WORD_MASK);
    A[8] -= 2;
    put ((int32_t *)&memory[A[8] & ADDRMASK], (int32_t) inst, (int32_t) WORD_MASK);
    A[8] -= 4;
    put ((int32_t *)&memory[A[8] & ADDRMASK], loc, LONG_MASK);
    A[8] -= 2;
    ...
    if (r_w == READ)
      infoWord |= 0x10;
    put ((int32_t *)&memory[A[8] & ADDRMASK], (int32_t)infoWord, (int32_t) WORD_MASK);/* push information word */
  }
