...
int ABCD()
{
    ...
    if (inst & 0x0008)
        put ((int32_t *)&memory[A[Rx]], result, (int32_t) BYTE_MASK);
    else
        put (&D[Rx], result, (int32_t) BYTE_MASK);
    if (carry)
        SR = SR | cbit;
    else
        SR = SR & ~cbit;

    cc_update (GEN, UND, CASE_1, UND, N_A, source, dest, result, (int32_t) BYTE_MASK, 0);
...
int	NBCD()
{
  ...
  put (EA1, result, (int32_t) BYTE_MASK);
  cc_update (GEN, UND, CASE_1, UND, N_A, source, dest, result, (int32_t) BYTE_MASK, 0);
