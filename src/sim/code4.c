...
// This code assumes int32_t is 32 bits and short is 16 bits
...
int	DIVS()
{
  ...
  from_2s_comp (EV1 & WORD_MASK, (int32_t) WORD_MASK, &source);
  ...
  if (result64 > 32767 || result64 < -32768)  // if overflow
    ...
    cc_update (N_A, GEN, GEN, N_A, ZER, source, dest, result, (int32_t) WORD_MASK, 0);
  }
...
// in the destination. The destination operand is a int32_t operand (32 bits) and
...
int	MULS()
{
  ...
  from_2s_comp (EV1 & WORD_MASK, (int32_t) WORD_MASK, &source);
  from_2s_comp (D[reg] & WORD_MASK, (int32_t) WORD_MASK, &dest);

  D[reg] = result = source * dest;

  cc_update (N_A, GEN, GEN, CASE_9, ZER, source, dest, result, (int32_t) LONG_MASK, 0);  // ck 8-10-2005
