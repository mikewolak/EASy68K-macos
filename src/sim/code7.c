...
int	SHIFT_ROT()
{
  ...
  if ((inst & 0xc0) == 0xc0) {
    int error = eff_addr((int32_t)WORD_MASK, MEM_ALT_ADDR, true);
...
{
  int	reg, mem_reg;
  uint32_t	size, bit_no;

  if (inst & 0x100)
    bit_no = D[(inst >> 9) & 0x07];
  else	{
    mem_request (&PC, (int32_t)WORD_MASK, (int32_t*)&bit_no); // get bit_no from instruction
    bit_no = bit_no & 0xff;
  }
  mem_reg = (inst & 0x38);
...
int	TAS()
{
  int error = eff_addr((int32_t)BYTE_MASK, DATA_ALT_ADDR, true);
  if (error)              // if address error
    return error;         // return error code

  /* perform the TAS operation */
  /* first set the condition codes according to *EA1 */
  cc_update (N_A, GEN, GEN, ZER, ZER, source, dest, *EA1, (int32_t) BYTE_MASK, 0);

  /* then set the high order bit of the *EA1 byte */
  put (EA1, EV1 | 0x80, (int32_t) BYTE_MASK);
...
int	BIT_FIELD()
{
  int32_t	extra, bf0, bf1, bf2, bf3, bf4;
  ...
  mem_request(&PC, (int32_t) WORD_MASK, &extra);
  if (extra & 0x800)
  ...
  if ((inst & 0x0038) == 0) {    // if Mode 000
    ...
    error = eff_addr((int32_t)BYTE_MASK, CONTROL_ADDR, true);
    if (error)              // if address error
      return error;         // return error code
    destAddr = (int32_t) ( (int32_t)EA1 - (int32_t)&memory[0]);  // destination address
    destAddr += (offset >> 3) | (offset & 0x80000000 ? ~0x1fffffff : 0);  // add offset
    error = mem_req (destAddr, (int32_t) BYTE_MASK, &bf0);  // get 5 bytes from memory
    if (error)
      return error;
    error = mem_req (destAddr+1, (int32_t) BYTE_MASK, &bf1);
    if (error)
      return error;
    error = mem_req (destAddr+2, (int32_t) BYTE_MASK, &bf2);
    if (error)
      return error;
    error = mem_req (destAddr+3, (int32_t) BYTE_MASK, &bf3);
    if (error)
      return error;
    error = mem_req (destAddr+4, (int32_t) BYTE_MASK, &bf4);
    if (error)
      return error;
  ...
  switch ((inst >> 8) & 0x7) {
    case BFCHG: case BFCLR: case BFSET: case BFINS:
      ...
      if ((inst & 0x0038) == 0) {               // if ea is data register
        ...
        mem_put(bf0 >> 24, destAddr,   (int32_t)BYTE_MASK);
        mem_put(bf0 >> 16, destAddr+1, (int32_t)BYTE_MASK);
        mem_put(bf0 >> 8,  destAddr+2, (int32_t)BYTE_MASK);
        mem_put(bf0,       destAddr+3, (int32_t)BYTE_MASK);
        if (((offset & 7) + width) > 32) {
          bf4 = (bf4 & (0xff >> (width - 32 + (offset & 7)))) |
                (tmp << (8 - (offset & 7)));
          mem_put(bf1, destAddr+4, (int32_t)BYTE_MASK);
        }
      }
  }
  return error;
