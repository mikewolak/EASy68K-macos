...
int	CHK()
{
  ...
  from_2s_comp (EV1, (int32_t) WORD_MASK, &source);
...
/*-------------------------------------------------------------------
 ...
 Trap codes 0 - 12 are designed to be compatible with the Teesside simulator.
  ...
  0 - Display string at (A1), D1.W bytes int32_t (max 255) with CRLF.
  1 - Display string at (A1), D1.W bytes int32_t (max 255) without CRLF.
