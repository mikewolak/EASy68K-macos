...
int	LEA()
{
  ...
  A[a_reg(reg)] = (int32_t) ((int32_t)EA1 - (int32_t)&memory[0]);
...
int	PEA()
{
  ...
  ea = ((int32_t)EA1 - (int32_t)&memory[0]);
  if ((inst & 0x003F) == 0x0038)  // if Abs.W  ck 2.9.3
    sign_extend ((int)ea, WORD_MASK, &ea);

  put ((int32_t*)&memory[A[a_reg(7)]], ea, LONG_MASK);
...
{
  int	reg;
  int32_t	temp, displacement;

  reg = inst & 0x07;

  mem_request (&PC, (int32_t) WORD_MASK, &temp);
  from_2s_comp(temp, (int32_t) WORD_MASK, &displacement);

  // perform the LINK operation
  A[a_reg(7)] -= 4;
  put ((int32_t *)&memory[A[a_reg(7)]], A[reg], LONG_MASK);
  A[reg] = A[a_reg(7)];
