...
int BCC()
{
  ...
  if (displacement == 0) {
    mem_request (&PC, (int32_t) WORD_MASK, &displacement);
    from_2s_comp (displacement, (int32_t) WORD_MASK, &displacement);
  } else
    from_2s_comp (displacement, (int32_t) BYTE_MASK, &displacement);
...
int	SCC()
{
  ...
  if (check_condition (condition))
    put (EA1, (int32_t) BYTE_MASK, (int32_t) BYTE_MASK);
  else
    put (EA1, (int32_t) 0, (int32_t) BYTE_MASK);
...
int	BSR()
{
  ...
  A[a_reg(7)] -= 4;
  put ((int32_t *) &memory[A[a_reg(7)]], PC, LONG_MASK);
...
int	JMP()
{
  int error = eff_addr((int32_t)WORD_MASK, CONTROL_ADDR, false);
  if (error)              // if address error
    return error;         // return error code
...
mem_request (&A[8], (int32_t) WORD_MASK, &temp);
SR = temp & WORD_MASK;
...
mem_request (&A[a_reg(7)], (int32_t) BYTE_MASK, &temp);
