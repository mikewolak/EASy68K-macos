...
#define MAXT 128                // maximum number of tokens
#define MAX_SIZE 512            // maximun size of input line
...
// Structured-assembly stacks live in structured.c; these reset them.
extern void stcStackClear(void);
extern void dbStackClear(void);
extern void forStackClear(void);

//------------------------------------------------------------
// Replace the extension of a path (Borland ChangeFileExt equivalent).
// 'ext' includes the leading dot, e.g. ".L68". If the file name has no
// extension the new one is appended.
static void changeFileExt(char *dst, const char *src, const char *ext)
{
  char *dot, *slash;
  strcpy(dst, src);
  slash = strrchr(dst, '/');           // last path separator (if any)
  dot   = strrchr(dst, '.');           // last dot
  if (dot && (!slash || dot > slash))  // a real extension after the directory
    *dot = '\0';                       // drop it
  strcat(dst, ext);
}
...
int assembleFile(char fileName[], char tempName[], const char *workName)
{
...

  {
    ...
    if (listFlag) {
      changeFileExt(outName, workName, ".L68");
      initList(outName);                        // initialize list file
    }

    // if Object file flag then create .S68 file (S-Record)
    if (objFlag) {
      changeFileExt(outName, workName, ".S68");
      if(initObj(outName) != NORMAL)             // if error initializing object file
    ...
    // notify the host (editor) of the final warning/error counts so it can
    // update its status display and enable the Execute action
    hostAssembleDone(warningCount, errorCount);
  }
