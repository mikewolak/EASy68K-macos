...
int printError(FILE *outFile, int errorCode, int lineNum)
{
  ...
  // Report the diagnostic to the host UI. In the original Borland build this
  // populated the active editor's clickable error list (and, for errors in an
  // include file, opened that file). The host now owns that presentation; the
  // core supplies the line number, message, and originating include file (if
  // any). An empty includeFile means the error is in the main source file.
  buffer[strlen(buffer)-1] = '\0';    // trim off trailing '\n'
  hostReportError(lineNum, buffer, includeFile);
